...
public class ControllerImpl implements Controller {
    ...
    public String addBeverage(String type, int counter, String brand, String name){
        Beverages beverage = type.equals("Fresh") ? new Fresh(name, counter, brand) : new Smoothie(name, counter, brand);
    ...
    public String reserve(int numberOfPeople) {
        ...
        Table table = tables.stream().filter(t -> !t.isReservedTable() && t.getSize() >= numberOfPeople).findFirst().orElse(null);

        //tables.stream().filter(t -> !t.isReservedTable() && t.getSize() >= numberOfPeople).findFirst().orElse(() -> new IllegalArgumentException("hello"));
