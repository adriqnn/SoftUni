...
public class House {
    ...
    public void removeCat(String name) {
        Cat catToRemove = this.cat.stream().filter(f -> name.equals(f.getName())).findAny().orElse(null);
    ...
    public Cat catForSale(String name){
        Cat cat = this.cat.stream().filter(f -> name.equals(f.getName())).findAny().orElse(null);
