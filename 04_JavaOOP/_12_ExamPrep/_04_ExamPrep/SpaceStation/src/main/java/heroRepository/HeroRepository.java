...
public class HeroRepository {
    ...
    public Hero getHeroWithHighestLevel() {
        Hero hero = this.data.stream().max(Comparator.comparingInt(Hero::getLevel)).orElse(null);

        return hero;
    }

    public Hero getHero(String name) {
        Hero hero = this.data.stream().filter(h -> h.getName().equals(name)).findFirst().orElse(null);
