...
public class Aquarium {
    ...
    public void remove(String name) {
        Fish fishToRemove = this.fish.stream().filter(f -> name.equals(f.getName())).findAny().orElse(null);
    ...
    public Fish sellFish(String name){
        Fish requestedFish = this.fish.stream().filter(f -> name.equals(f.getName())).findAny().orElse(null);
