...
const contextAPI = {
    ...
    storeMyTeams: async (context, next) => {
        const [myTeams, allMembers] = await Promise.all([ teams.getAllTeamsOfUser(sessionStorage.getItem('id')), teams.getAllMembers() ]);
        context.myTeams = myTeams.map(x => ({ ...x, members: allMembers.filter(y => y.teamId === x.team._id) }));
        next();
    }
}
