...
const update = async (context, e) => {
    ...
    try{
        ...
        render(PageLayout(EditView(context, errors[error.message])), document.getElementById('content'));
    }
}
...
const EditView = (context, errorMsg) => {
    return html `<section id="edit">
                     <article class="narrow">
                         ...
                         <form id="edit-form" class="main-form pad-large" @submit="${(e) => update(context, e)}">
                             ${errorMsg ? html `<div class="error">${errorMsg}</div>` : nothing}
                             <label>Team name: <input type="text" name="name" .value="${context.team.name}"></label>
                             <label>Logo URL: <input type="text" name="logoUrl" .value="${context.team.logoUrl}"></label>
                             <label>Description: <textarea name="description" .value="${context.team.description}"></textarea></label>
