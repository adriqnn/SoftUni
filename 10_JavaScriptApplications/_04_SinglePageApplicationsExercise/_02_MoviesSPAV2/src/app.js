...
import { deserializeFormData, isValidInput, clearFormFields, checkServerError, parseMovieData } from './helper.js';
import { addLike, createMovie, getIfUserLikedMovie, getMovieData, getMovieLikesNumber, loginRequest, logoutRequest, registerRequest, removeLike, updateMovie, deleteMovie } from './requests.js';
...
document.addEventListener('click', e => {
    const buttonsActions = {
        ...
        likeMovieBtn: async () => {
            ...
            like ? await removeLike(like._id) : await addLike({ movieId, ownerId });
    ...
    if((e.target.tagName === 'BUTTON' || e.target.tagName === 'A') && e.target.type !== 'submit'){
        ...
        const data = deserializeFormData(form);
