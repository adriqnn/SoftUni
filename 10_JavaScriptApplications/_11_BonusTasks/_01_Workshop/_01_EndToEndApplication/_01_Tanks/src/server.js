    ...
    io.on('connection', socket => {
        ...
        socket.on('join', ({ username: newUsername, roomId: newRoomId, player }) => {
            ...
            if(rooms[roomId] === undefined){
                rooms[roomId] = { projectiles: [], players: new Map() };
            }
        ...
        Object.entries(rooms).forEach(([roomId, room]) => {
            ...
            for(let shot of room.projectiles){
                ...
                for (let [username, player] of [...room.players.entries()]) {
                    if(shot.origin !== username && shot.x >= player.x - TANK_SIZE && shot.x <= player.x + TANK_SIZE && shot.y >= player.y - TANK_SIZE && shot.y <= player.y + TANK_SIZE){
            ...
            io.to(roomId).emit('update', { projectiles: room.projectiles, players: [...room.players.entries()], hits });
        });
    }
}
