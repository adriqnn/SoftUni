...
//                " AND COLUMN_NAME != '%s'" + " AND TABLE_NAME = '%s';";
...
//        String tableName = getTableName(table);
//        String selectQuery = String.format("SELECT * FROM %s %s",tableName,where != null ? " WHERE " + where : "");
//
//        PreparedStatement statement = connection.prepareStatement(selectQuery);
//        ResultSet resultSet = statement.executeQuery();
//        List<E> result = new ArrayList<>();
...
//            declaredField.set(entity,value);
//        }else if(fieldType == long.class || fieldType == Long.class){
//            long value = resultSet.getInt(fieldName);
//            declaredField.set(entity,value);
//        }else if(fieldType == LocalDate.class){
//            LocalDate value = LocalDate.parse(resultSet.getString(fieldName));
//            declaredField.set(entity,value);
...
//
//    private Field getIdColumn(Class<?> clazz){
//        return Arrays.stream(clazz.getDeclaredFields()).filter(f -> f.isAnnotationPresent(Id.class)).findFirst().orElseThrow(() ->  new UnsupportedOperationException("Entity missing an Id column"));
