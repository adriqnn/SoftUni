...
public class SellerServiceImpl implements SellerService {
    ...
    public String importSellers() throws IOException, JAXBException {
        ...
        xmlParser.fromFile(SELLERS_FILE_PATH,SellerSeedRootDTO.class).getSellers().stream().filter(sellerSeedDTO ->{
            boolean isValid = validationUtil.isValid(sellerSeedDTO);
            sb.append(isValid ? String.format("Successfully import seller %s - %s",sellerSeedDTO.getLastName(),sellerSeedDTO.getEmail()) : "Invalid seller").append(System.lineSeparator());
            return isValid;
