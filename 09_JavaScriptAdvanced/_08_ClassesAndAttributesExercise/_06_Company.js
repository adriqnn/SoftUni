class CompanyV1{
    ...
    bestSalaryDepart = () => this.departments.sort((a, b) => this.getSalariesSum(b) / b.salaries.length - this.getSalariesSum(a) / a.salaries.length)[0];
    ...
    bestDepartment(){
        ...
        const printData = best.employees.reduce((a, v, i) => {
            ...
            return a;
        }, []).sort((a, b) => b[1] - a[1] === 0 ? a[0].localeCompare(b[0]) : b[1] - a[1]).map(x => x.join(' ')).join('\n');
