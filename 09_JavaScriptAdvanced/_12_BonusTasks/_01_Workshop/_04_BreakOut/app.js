...
const ball = { x: 400, y: 300, vel: getVector(speed, Math.PI / 4) };
const ballSize = 10;
...
function tick(){
    ...
    // paddle hit
    if ((ball.y > limits.bottom && ball.vel.y > 0) && (ball.y <= limits.bottom + ballSize) && (ball.x >= mouse.x - padWidth / 2 - ballSize) && ball.x <= mouse.x + padWidth / 2 + ballSize){
        ball.vel.y *= -1;
...
function main(time){
    ...
    render();
    // ctx.fillText(time.toFixed(2), 20, 20);
