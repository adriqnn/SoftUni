...
public class _08_RoundingNumbers
{
    public static void Main(string[] args)
    {
        ...
        string result = string.Join(Environment.NewLine, Console.ReadLine().Split().Select(double.Parse).ToArray().Select(x => $"{x} => {Math.Round(x, MidpointRounding.AwayFromZero)}"));
