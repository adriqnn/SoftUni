...
public class _07_CrossfireV2 {
    public static void main(String[] args) throws IOException {
        ...
        while (!line.equals("Nuke it from orbit")){
            ...
            // Optimize the removal process using iterators
            for (int i = row - radius; i <= row + radius; i++) {
                if (isInRange(i, col, matrix) && i != row) {
                    int finalI = i;
                    matrix.get(i).removeIf(x -> Objects.equals(x, matrix.get(finalI).get(col)));
                }
            }
            ...
            for (int i = col + radius; i >= col - radius; i--) {
                if (isInRange(row, i, matrix)) {
                    int finalI = i;
                    matrix.get(row).removeIf(x -> Objects.equals(x, matrix.get(row).get(finalI)));
                }
            }
